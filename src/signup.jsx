...
export default function Singup() {
    ...
    const handleConfrimPass = (evt) => {
        setConfirmPass(evt.target.value)
    }
    const handleCheck = () => {
        if (pass === confirmPass) {
            axios.post(process.env.REACT_APP_SIGNUP_API_URI, { name: name, password: pass })
                .then((response) => {
                    const { data } = response
                    if (data === true) {
                        navigate('/success')
                ...
                <div className="flex flex-col ">
                    ...
                    <div className="flex justify-between">
                        <div className="flex gap-2 px-2 items-center pb-4">
                            <button style={{boxShadow:"rgba(0, 0, 0, 0.24) 0px 3px 8px"}} className="text-white font-medium bg-[#278bb0] rounded-b-xl p-1 text-xs md:text-xl " onClick={handleLogin}>Login</button>
                            <span style={{backgroundColor:mode?"white":"black", color:mode?"black":"white"}} className="smooth-transition font-medium rounded-b-lg p-1 text-xs md:text-xl">Signup</span>
                        </div>
                    ...
                    <div className="md:flex flex-col justify-center items-center h-full font-medium px-2 md:px-6">
                        <table >
                            <tr className="pb-10">
                            ...
                            <tr>
                                <td></td>
                                <td><button style={{backgroundColor:mode?"white":"black", color:mode?"black":"white"}} className="smooth-transition text-white px-3 py-1 mb-3 text-xs md:text-xl" onClick={handleCheck}>Signup</button></td>
                            </tr>
                        </table>
                        <div style={{ display: alert ? "block" : "none" }} className=" flex border bg-red-100 rounded-xl border-red-500 p-1 md:flex-col gap-1 justify-center text-center mt-2">
                            <h1 className="text-xs md:text-xl font-medium text-red-500">User Already Exists </h1>
                            <h1 className="text-xs md:text-xl font-medium text-red-500">Please Again with different User!</h1>
                        </div>
                    </div>
